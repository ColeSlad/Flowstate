...
int main(int argc,char** argv) {
    try {
        ...
        for(int i=1;i<argc;++i) {
            ...
            if(key=="--help") {
                std::cout<<"--trace CSV --mode heuristic|cpu_latency|gpu_immediate|gpu_batch|balanced\n"
            ...
            else if(key=="--interval-ms") interval_ms=fs::cli::number(value);
        ...
        const auto static_policy=mode=="heuristic"?fs::Policy::CpuLatency:fs::parse_policy(mode);
        ...
        std::unique_ptr<fs::HeuristicController> controller;
        if(mode=="heuristic") controller=std::make_unique<fs::HeuristicController>(runtime,thresholds,std::chrono::milliseconds(interval_ms));
        ...
        if(!telemetry_path.empty()) {
            ...
            telemetry<<"elapsed_ms,phase,policy,arrival_rate,throughput,queue_depth,pending_gpu_jobs,cpu_utilization,gpu_utilization,gpu_memory_free_bytes,p50_ms,p95_ms,p99_ms,scalar_p95_ms,simd_p95_ms,cuda_p95_ms,gpu_batch_p95_ms,mean_gpu_batch_size,policy_changes,controller_errors,sample_age_ms\n";
        }
